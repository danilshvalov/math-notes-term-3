...
% #+begin_definition
% Если \(\rang A = r\), то \(\forall\) ненулевой минор порядка \(r\) называется базисным минором, а его строки (столбцы) — базисными.
% #+end_definition

% #+attr_latex: :options [о базисном миноре]
% #+begin_theorem
% 1. Строки (столбцы) базисного минора линейно независимы.
